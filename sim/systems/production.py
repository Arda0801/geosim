def produce(facility, world):
    ...
    # Check whether we have enough inputs (from REGION inventory)
    ...
    if possible_production <= 0:
        return 0.0
    # Consume inputs (from REGION inventory)
    for commodity_id, required_per_unit in facility.inputs.items():
    ...
    # Produce outputs — into the COMPANY's inventory (for selling via market)
